...
# You can't die yet, add function when ...
#   - Touch ur body (X AND Y is the same)
#   - X AND Y are outside boundary
# Also add pause time in beginng, start when user touches a key **
...
class Square:
    ...
    def drawself(self, turtle):
        # draw a black box at its coordinates, leaving a small gap between
        # cubes
...
class Food:
    ...
    def changelocation(self):
        # Change!
        # make it so that it can spawn outside of where the snakes body is!
...
class Game:
    ...
    def nextFrame(self):
        while True: 
            ...
            self.artist.clear()
            if self.counter == 5: 
            # only moves to next frame every 5 loops, this was an attempt to
            # get rid of the turning delay
